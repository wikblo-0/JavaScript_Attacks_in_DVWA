...
SECURITY="high" #security level in DVWA
...
REV=$(echo "$PHRASE" | rev) #step 1

STEP2=$(echo -n "XX$REV" | sha256sum | awk '{print $1}') #step 2

TOKEN=$(echo -n "$STEP2""ZZ" | sha256sum | awk '{print $1}') #step 3
